  ...
  try {
    ...
    const artworks = await Artwork.findAll({
      ...
      attributes: [
        ...
        [sequelize.literal(`EXISTS (SELECT 1 FROM FavoriteArtworks WHERE FavoriteArtworks.artwork_id = Artwork.id AND FavoriteArtworks.user_id = ${userId})`), 'isLiked']
      ],
    ...
    const artworksData = artworks.map(artwork => {
      ...
      return {
        ...
        isLiked: plainArtwork.isLiked
      };
    });

    // 2. 진행 중인 경매 정보 가져오기{좋아요순)
    const ongoingAuctions = await Auction.findAll({
      ...
      attributes: [
        ...
        [sequelize.literal(`EXISTS (SELECT 1 FROM FavoriteAuctions WHERE FavoriteAuctions.auction_id = Auction.id AND FavoriteAuctions.user_id = ${userId})`), 'isLiked']
      ],
      include: [{
        model: Artwork,
        as: 'artwork',
